# Apply improved edge-based path extraction + per-path sampling + junction margin + global thinning
# to the bird image. Spacing = 20 px on this image.
...
def edge_paths(coords, adj, deg):
    ...
    for u in range(len(coords)):
        for v in adj[u]:
            ...
            while deg[curr]==2:
                # continue along non-prev neighbor
...
if len(pts_clean): plt.scatter(pts_clean[:,0], pts_clean[:,1], s=10, c='orange')
